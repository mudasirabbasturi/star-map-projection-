...
// SVG Text Editor — Now with PDF export (vector or raster fallback)
// Uses jsPDF + svg2pdf when available; falls back to high-res raster PDF if svg2pdf isn't present.
// Note: In your build, install jspdf and svg2pdf.js if you want true vector PDFs:
...
export default function SvgPaperEditor() {
  ...
  // PDF export: try vector (svg2pdf) first, else fallback to raster PDF
  async function downloadPDFVector() {
    const svg = svgRef.current;
    if (!svg) return await downloadPDFRaster();
    try {
      // dynamic import so build doesn't fail if libs aren't installed
      const { jsPDF } = await import("jspdf");
      const svg2pdf =
        (await import("svg2pdf.js")).default || (await import("svg2pdf.js"));

      // create jsPDF with correct unit = mm and paper size
      const paperDims = PAPERS[paper];
      const pdf = new jsPDF({ unit: "mm", format: [paperDims.w, paperDims.h] });

      // clone svg and remove UI helpers
      const clone = svg.cloneNode(true);
      clone.querySelectorAll("[data-ui]").forEach((n) => n.remove());

      // embed fonts? jsPDF+svg2pdf will use available system fonts; for perfect fidelity, convert text to paths server-side.

      // svg2pdf expects an SVGElement and a PDF document
      svg2pdf(clone, pdf, { xOffset: 0, yOffset: 0, scale: 1 });

      pdf.save(`${paper}-design.pdf`);
      return true;
    } catch (err) {
      console.warn(
        "Vector PDF export failed, falling back to raster PDF:",
        err
      );
      return await downloadPDFRaster();
    }
  }

  async function downloadPDFRaster(dpi = 300) {
    // rasterize SVG to canvas at high DPI and put into PDF as image
    const svg = svgRef.current;
    if (!svg) return;
    const serializer = new XMLSerializer();
    const svgString = serializer.serializeToString(svg);
    ...
    return new Promise((resolve) => {
      img.onload = async () => {
        ...
        try {
          ...
          // calculate image size in mm to fill page
          const pxPerMm = (96 / 25.4) * scale; // approx px per mm
          const imgWmm = Math.round((canvas.width / pxPerMm) * 100) / 100;
          const imgHmm = Math.round((canvas.height / pxPerMm) * 100) / 100;
          pdf.addImage(imgData, "PNG", 0, 0, imgWmm, imgHmm);
          pdf.save(`${paper}-design.pdf`);
          URL.revokeObjectURL(url);
          resolve(true);
        } catch (err) {
          console.warn("PDF raster export failed:", err);
          URL.revokeObjectURL(url);
          resolve(false);
        }
      };
  ...
  function downloadPDF() {
    // Try vector version first, fallback to raster
    downloadPDFVector();
  }

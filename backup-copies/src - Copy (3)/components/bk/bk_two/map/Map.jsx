...
// Convert longitude -> center RA: using simple mapping used earlier (negate lon)
const centerRAFromLon = (lon) => {
  // keep in degrees -180..+180
  let v = -lon;
  while (v <= -180) v += 360;
  while (v > 180) v -= 360;
  return v;
...
const Map = ({
  ...
  showMoon = true,
  sizeMult = 1,
  centerRA = 0, // kept for backward compat; we compute derivedCenterRA from lon if lat/lon provided
  strokeColor = "#eee",
  strokeWidth = 1,
  strokeStyle = "solid",
  fill,
  lat = 0,
  lon = 0,
  milkywayOpacity = 0.2, // 0..1
  magLimit = 6.5, // magnitude limit: brighter stars have lower magnitudes
}) => {
  ...
  // Derived center RA/Dec from lat/lon
  // centerRA: rotate sky horizontally; centerDec: vertical offset
  const derivedCenterRA = centerRAFromLon(lon);
  const derivedCenterDec = clamp(lat, -90, 90);

  // --- Stars: respect magLimit and sizeMult ---
  useEffect(() => {
    ...
    for (const f of starsData.features) {
      ...
      // Skip stars dimmer than limit (larger magnitudes are dimmer)
      if (mag > magLimit) continue;

      const [ra, dec] = f.geometry.coordinates;
      // shift declination by centerDec: effectively show centerDec at vertical middle
      // We keep using the existing raDecToXY helper, passing derivedCenterRA.
      // To apply centerDec shift we subtract it from the star's dec before mapping.
      const { x, y } = raDecToXY(ra, dec - derivedCenterDec, derivedCenterRA);

      // quick off-screen cull (same as before)
      ...
      // opacity mapping: clamp into [0.05, 1]
      // (1.2 - mag/6.5) was used before; keep similar behaviour but clamp
  ...
  // --- Milky Way (honour milkywayOpacity and showMilkyway) ---
  useEffect(() => {
    const svg = svgRef.current;
    if (!svg) return;
    ...
    for (const f of mwData.features) {
      ...
      for (const poly of polygons) {
        for (const ring of poly) {
          ...
          // Use provided opacity (clamped 0..1)
          path.setAttribute(
            "fill",
  ...
  // --- Constellations (honour showConstellations) ---
  useEffect(() => {
    const svg = svgRef.current;
    if (!svg) return;
    ...
    for (const f of constData.features) {
      ...
      if (!labelPos) {
        // fallback: midpoint of first line
        const firstLine =
  ...
  // --- Planets & Moon (use Astronomy observer built from lat/lon) ---
  useEffect(() => {
    const svg = svgRef.current;
    if (!svg) return;
    ...
    // Build observer with lat/lon (altitude 0)
    ...
    if (showPlanets) {
      ...
      for (const p of bodies) {
        try {
          ...
          // eq.ra is in hours (0..24), convert to degrees:
          ...
          // ignore failures for individual bodies
          // console.warn("planet failed", p.name, err);
        }
      }
    }
